...
void usage(const char *cmd)
{
    ...
    printf("  -E <error count theshold>, if import error count more than this num, exit\n");
...
#endif
	while ((c = getopt(argc,argv,"f:p:a:m:s:S:g:c:r:e:E:M:K:vh")) != -1) {
		switch (c) {
			case 'f':
			...
			case 'e':
				...
				break;
            case 'E':
                config.errorTheshold = atoi(optarg);
                break;
            case 'K':
